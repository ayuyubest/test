...
# 加载测试数据
def load_predict_config_test_data():
    import os
    # 获取当前文件所在目录的父目录，然后拼接data目录路径
    current_dir = os.path.dirname(os.path.dirname(__file__))
    csv_path = os.path.join(current_dir, "data", "test_cases_fixed.csv")
    df = pd.read_csv(csv_path)
    return [case for case in df.to_dict("records") if 'predict_config' in case.get('tags', '')]

@allure.feature("预测配置")
class TestPredictConfig:
    
    @pytest.mark.parametrize("test_case", load_predict_config_test_data())
    def test_predict_config_api(self, api_client, auth_cookies, test_case: Dict[str, Any]):
        """
        预测配置相关API测试
        """
        # 设置allure标签
