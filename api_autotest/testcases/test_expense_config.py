...
# 加载测试数据
def load_expense_config_test_data():
    import os
    # 获取当前文件所在目录的父目录，然后拼接data目录路径
    current_dir = os.path.dirname(os.path.dirname(__file__))
    csv_path = os.path.join(current_dir, "data", "test_cases_fixed.csv")
    df = pd.read_csv(csv_path)
    return [case for case in df.to_dict("records") if 'expense_config' in case.get('tags', '')]

@allure.feature("费用项配置")
class TestExpenseConfig:
    
    @pytest.mark.parametrize("test_case", load_expense_config_test_data())
    def test_expense_config_api(self, api_client, auth_cookies, test_case: Dict[str, Any]):
        """
        费用项配置相关API测试
        """
        # 设置allure标签
